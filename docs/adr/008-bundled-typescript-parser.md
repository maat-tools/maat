...
maat bundles its own TypeScript parser via `ts-morph`. The `@maat/collector-ts` package depends on `ts-morph` directly. The target project's installed `typescript` version is not consulted.
...
- `@maat/collector-ts` carries its own `typescript` version in `devDependencies`. Running `maat check` on a project using TypeScript 5.8 while maat bundles TypeScript 5.4 is explicitly supported.
