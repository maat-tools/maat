...
A ledger backend persists events and rebuilds the current ledger snapshot.

Official ledger backends preserve append-only history. The current file ledger writes NDJSON events by appending lines and derives state by replaying those events. The ledger file is generated by Maat, but it is meant to be committed to Git when you want findings, accepted exceptions, and axiom history to travel with the repository.
