...
export class Axiom extends MaatCommandBase implements MaatCommand {
    ...
    private async declare(options: DeclareOptions) {
        ...
        const fingerprints = options.fingerprints
            ? options.fingerprints.split(',').map((s) => s.trim()).filter(Boolean)
            : undefined;
    ...
    private async lifecycle(
        type: typeof FindingStatus.AXIOM_SUPERSEDED | typeof FindingStatus.AXIOM_REVOKED,
        ...
        const verb = type === FindingStatus.AXIOM_SUPERSEDED ? 'superseded' : 'revoked';
