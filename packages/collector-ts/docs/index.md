...
Built using [Jelly](https://github.com/cs-au-dk/jelly) for whole-program points-to analysis. `nodes` are named callable units; `edges` are directed caller-to-callee relationships.
...
| `callGraph.timeout` | — | Timeout in seconds for Jelly's analysis pass |
...
- The call graph is built by Jelly separately from the ts-morph pass. If Jelly is not installed or the analysis times out, `callGraph` is returned empty without failing the run.
